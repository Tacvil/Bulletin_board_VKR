...
import com.example.bulletin_board.databinding.ControlsLayoutBinding
import com.example.bulletin_board.databinding.FragmentPixBinding
import com.example.bulletin_board.databinding.GridLayoutBinding
import com.example.bulletin_board.databinding.PermissionsLayoutBinding
import com.example.bulletin_board.databinding.VideoCounterLayoutBinding
...
import com.example.bulletin_board.pix.models.Img
...
class PixFragment(private val resultCallback: ((PixEventCallback.Results) -> Unit)? = null) :
    ...
    private fun FragmentActivity.setup() {
        setUpMargins(binding) //типо отступ плашки камеры и тд от низа
        permissions()
        reSetup(this)
    ...
    private fun setupControls() {
        binding.setupClickControls(model, cameraXManager, options) { int, uri ->
            when (int) {
                ...
                5 -> if (model.longSelectionValue){
                    binding.gridLayout.sendButtonStateAnimation(true)
                }
